...
def score_position(board, piece):
    ...
    # Add more scoring for horizontal, vertical, and diagonal patterns
    return score
...
#def pick_best_move(board, piece):
    # Pick the best move based on the board evaluation
...
board = create_board()
game_over = False
turn = random.randint(PLAYER_PIECE, AI_PIECE)  # Randomize starting turn
...
while not game_over:
    for event in pygame.event.get():
        ...
        if turn == AI_PIECE and not game_over:
            ...
            if is_valid_location(board, col):
                pygame.time.wait(500)  # AI delay for realism
                row = get_next_open_row(board, col)
